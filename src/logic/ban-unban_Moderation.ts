...
export default
async (s: ChatInputCommandInteraction | Message, ban: 'BAN' | 'UNBAN') => {
        ...
        return await s.reply({
            embeds: [
                new EmbedBuilder().setDescription(
                    `I cannot ban ${member.user.tag}! They may have higher or equal roles, or I lack permissions.`
                ),
            ],
        });
    ...
    if (ban === 'BAN') {
        ...
        return await s.reply({
            embeds: [
                new EmbedBuilder()
                    .setDescription(`You have banned ${member}.\nReason: ${reason}`)
            ]
        });
    }
    else
        return await s.reply({
            embeds: [
                new EmbedBuilder()
                    .setDescription('`unban` command is not yet implemented!')
            ]
        });
}
